...
<?php
  ...
  $display = "<table id='packages' style='width:50%;' align='center'>"
           . "<thead>"
           . "<tr><th colspan='3'>INDIVIDUAL PACKAGE BILL</th></tr>"
           . "<tr><td colspan='3' bgcolor='#2EFEF7'></td></tr>"
           . "<tr><th colspan='3'>PACKAGES</th></tr>"
           . "</thead><tbody>";

  foreach($packages as $pid=>$package_name){
  	$display = $display."<tr><td  bgcolor='#084B8A' colspan=3><button onclick=\"javascript=window.location='individual_package_events.php?pid=$pid&uid=$uid'\">$package_name</a></button></td></tr>";
        $info = $events[$pid];
        $count = count($info);
